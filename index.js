...
import makeWASocket, { DisconnectReason, useMultiFileAuthState, fetchLatestBaileysVersion, makeInMemoryStore } from '@whiskeysockets/baileys' import fs from 'fs' import P from 'pino' import qrcode from 'qrcode-terminal' import dotenv from 'dotenv' import axios from 'axios' import moment from 'moment-timezone' dotenv.config()

const prefix = process.env.PREFIX || '!' const OWNER = process.env.OWNER_NUMBER || '255624236654' const BOT_NAME = process.env.BOT_NAME || 'Ben Whittaker Tech' const store = makeInMemoryStore({ logger: P().child({ level: 'silent', stream: 'store' }) })

let votes = {}

async function startBot() { const { state, saveCreds } = await useMultiFileAuthState('auth') const { version } = await fetchLatestBaileysVersion() const sock = makeWASocket({ version, printQRInTerminal: true, auth: state, logger: P({ level: 'silent' }), browser: ['Ben Whittaker Tech', 'Safari', '1.0.0'] })

store.bind(sock.ev) sock.ev.on('creds.update', saveCreds) sock.ev.on('connection.update', ({ connection, lastDisconnect }) => { if (connection === 'close' && lastDisconnect?.error?.output?.statusCode !== DisconnectReason.loggedOut) { startBot() } else if (connection === 'open') { console.log(\n✅ ${BOT_NAME} connected!) } })

sock.ev.on('messages.upsert', async ({ messages }) => { const msg = messages[0] if (!msg.message || msg.key && msg.key.remoteJid === 'status@broadcast') return
